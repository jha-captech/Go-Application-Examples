...
// Ensure that moqvalidator does implement validator.
// If this is not the case, regenerate this file with mockery.
var _ validator = &moqvalidator{}

// moqvalidator is a mock implementation of validator.
//
//	func TestSomethingThatUsesvalidator(t *testing.T) {
//
//		// make and configure a mocked validator
//		mockedvalidator := &moqvalidator{
//			ValidFunc: func(ctx context.Context) map[string]string {
//				panic("mock out the Valid method")
//			},
//		}
//
...
type moqvalidator struct {
	// ValidFunc mocks the Valid method.
	ValidFunc func(ctx context.Context) map[string]string

	// calls tracks calls to the methods.
	calls struct {
		// Valid holds details about calls to the Valid method.
		Valid []struct {
			// Ctx is the ctx argument value.
			Ctx context.Context
		}
	}
	lockValid sync.RWMutex
}

// Valid calls ValidFunc.
func (mock *moqvalidator) Valid(ctx context.Context) map[string]string {
	if mock.ValidFunc == nil {
		panic("moqvalidator.ValidFunc: method is nil but validator.Valid was just called")
	}
	callInfo := struct {
		Ctx context.Context
	}{
	...
	mock.lockValid.Lock()
	mock.calls.Valid = append(mock.calls.Valid, callInfo)
	mock.lockValid.Unlock()
	return mock.ValidFunc(ctx)
}

// ValidCalls gets all the calls that were made to Valid.
// Check the length with:
//
//	len(mockedvalidator.ValidCalls())
func (mock *moqvalidator) ValidCalls() []struct {
	Ctx context.Context
} {
	var calls []struct {
		Ctx context.Context
	}
	mock.lockValid.RLock()
	calls = mock.calls.Valid
	mock.lockValid.RUnlock()
	return calls
}
